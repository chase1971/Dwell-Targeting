    ...
    internal static List<object> BuildList(Type entryType, Type configTypeEnum)
    {
        var list = new List<object>
        {
            ...
            Toggle("enablePerfLogging", "Performance Logging", SettingsStore.Current.EnablePerfLogging,
                ...
                entryType, configTypeEnum),
            Slider("treeScanIntervalFrames", "Tree Scan Interval (frames)", SettingsStore.Current.TreeScanIntervalFrames,
                "Frames between scene-tree rescans. Lower = snappier overlays, higher = less CPU. Default 45 (~0.75s at 60 FPS). Turn on Performance Logging to tune.",
                v => SettingsStore.ApplyTreeScanIntervalFrames(Convert.ToSingle(v), persist: true, syncModConfig: false),
                DwellSettings.MinTreeScanIntervalFrames, DwellSettings.MaxTreeScanIntervalFrames, 5f, "F0", entryType, configTypeEnum),
            Toggle("showHitboxOverlay", "Show Hitbox Outlines", SettingsStore.Current.ShowHitboxOverlay,
                "Draw green outlines on dwell targets while overlay visuals are ON (use the ON/OFF button beside the map to hide all visuals). Dwell still works when hidden.",
            ...
            Slider("endTurnDwellSeconds", "End Turn Hover Time", SettingsStore.Current.EndTurnDwellSeconds,
                "Center E overlay and the native End Turn button in the bottom-right.",
                v => SettingsStore.ApplyEndTurnDwellSeconds(Convert.ToSingle(v), persist: true, syncModConfig: false),
                DwellSettings.MinDwellSeconds, DwellSettings.MaxDwellSeconds, 0.05f, "F2", entryType, configTypeEnum),
            Slider("menuDwellSeconds", "Menu & Utility Hover Time", SettingsStore.Current.MenuDwellSeconds,
                "Everything else: pause/title menus, utility bar, Confirm, event/map proceed, offset numbers, back button, etc.",
                v => SettingsStore.ApplyMenuDwellSeconds(Convert.ToSingle(v), persist: true, syncModConfig: false),
                DwellSettings.MinDwellSeconds, DwellSettings.MaxDwellSeconds, 0.05f, "F2", entryType, configTypeEnum),
            ...
            Slider("menuButtonOpacity", "Button Opacity", SettingsStore.Current.MenuButtonOpacity,
                ...
                DwellSettings.MinOpacity, DwellSettings.MaxOpacity, 0.05f, "F2", entryType, configTypeEnum),
            Toggle("hideEndTurnButton", "Hide Center End Turn Overlay", SettingsStore.Current.HideEndTurnButton,
                "Hide the large center E END overlay. Native End Turn button dwell still works.",
                v => SettingsStore.ApplyHideEndTurnButton(Convert.ToBoolean(v), persist: true, syncModConfig: false),
                entryType, configTypeEnum),
            Toggle("hideConfirmButton", "Hide Center Confirm Overlay", SettingsStore.Current.HideConfirmButton,
                "Hide the mod E/OK overlay during hand selection. The game's Accept button still works with hover dwell.",
